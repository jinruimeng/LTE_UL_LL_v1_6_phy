function coded_blocks = LTE_UL_tx_turbo_encode(LTE_params,code_blocks,UE_signaling,stream_index)
% LTE turbo encoder.
...
% last changes:
%   2010/11/11 
%   in case of uplink more than one UE instances, needs to be modified
%   for UE indexing, 
% added Prokopec, Blumenstein, 11.11.2011

size(code_blocks,2);
for i=1:size(code_blocks,2)
    % in case of uplink more than one UE instances, needs to be modified
    % for UE indexing, 
    % added Prokopec, Blumenstein, 11.11.2011
    
    coded_blocks{i} = LTE_tx_turbo_encode_code_block(LTE_params,code_blocks{i},i,UE_signaling.TB_segmentation_UL(stream_index));
    UE_signaling.turbo_encoder_UL(stream_index).encoded_CB_sizes(i) = length(coded_blocks{i});
