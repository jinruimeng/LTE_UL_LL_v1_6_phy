...
LTE_params.nUE = 1;                         % number of user equipments to simulate, currently only one is supported
LTE_params.nBS = 1;                         % number of base stations to simulate, currently only one is supported
...
LTE_params.to_plot = {'bler_user','throughput_user'};%{'bler_user', 'throughput_user', 'throughput_cell', 'papr'}; %leave empty to plot all
LTE_params.plot_confidence_intervals = true; % deprecated -> remove?
LTE_params.save_plots = false; % should the plots be saved as images

%% Define User parameters (identical settings)
LTE_params.UE_config.mode = 1;                      % DEFINED IN STANDARD 3GPP TS 36.213 V11.4.0 Section 8.0, page 105
...
LTE_params.BS_config.channel_interpolation_method = 'flat';         % 'flat','linear','pchip','spline'
...
LTE_params.BS_config.channel_estimation_time_smoothing = 0;         % time smoothing contraint, currently not in use
LTE_params.BS_config.channel_estimation_frequency_smoothing = 8;    % frequency smoothing constraint for LS_QS channel estimation, 8
...
LTE_params.ChanMod_config.type                  = 'PedA';%flat Rayleigh';                         % 'PedA', 'PedB', 'PedBcorr', 'AWGN', 'flat Rayleigh', 'Rayleigh', 'VehA','VehB','TU','RA','HT','winner_II', 'ePDP'
...
LTE_params.scheduler.fixed_scheduler_assignment = [0 3 3];        % RB assignment for the fixed scheduler
