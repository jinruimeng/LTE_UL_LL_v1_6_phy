function dk = LTE_UL_rx_turbo_rate_matcher(LTE_params,ek,UE_signaling,UE,stream_index)
...
% C = length(ek);
C = UE_signaling.TB_segmentation_UL(stream_index).C;
% C = 1;
...
for i=1:C
    ...
    bit_selection_and_pruning_mapping = UE_signaling.turbo_rate_matcher_UL(stream_index).bit_selection_and_pruning_mapping{i};
    wk{i} = LTE_common_turbo_rate_matching_bit_selection_and_pruning(double(ek{i}),...
        bit_selection_and_pruning_mapping,...
        2,...
        UE_signaling.turbo_rate_matcher_UL(stream_index).subblock_interleaver(i).K_pi*3);
    % Circular buffer
    ...
    UE_signaling.turbo_rate_matcher_UL(stream_index).ek_sizes(i) = length(bit_selection_and_pruning_mapping);
end
% Sub-block interleaver
dk = subblock_interleaver(LTE_params,UE_signaling.TB_segmentation_UL(stream_index).C,UE_signaling.turbo_rate_matcher_UL(stream_index),vk);
end
