classdef downlinkChannel < handle
% Class that represents the downlink channel. Basically a buffer containing N
% feedback outputs, which delays the UE feedback a specified amount of
% TTIs.
...
% www.nt.tuwien.ac.at
   ...
   methods
       ...
       function obj = downlinkChannel(delay,nUEs,nBS)
           ...
           obj.buffer = outputs.ueOutput(nUEs*nBS);

           for u_ = 1:nUEs*nBS                  % Over UEs
               for d_ = 1:obj.buffer_length     % Over delay
                   obj.buffer(d_,u_) = outputs.ueOutput(nUEs*nBS);
               end
           end 
       end
