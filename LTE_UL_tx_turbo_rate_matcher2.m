function ek = LTE__UL_tx_turbo_rate_matcher2(LTE_params,dk,UE_signaling,UE,stream_index)
...
LTE_params.balanced_rate_matching = false;
% LTE_params.null_bit = 3;
LTE_params.null_bit = 3;
UE_signaling.turbo_rate_matcher_UL(stream_index).total_bits_before_punturing = 0;
C = length(dk);
vk = cell(1,C);
wk = cell(1,C);
ek = cell(1,C);
...
[   vk,... % cell array v_k
    UE_signaling.turbo_rate_matcher_UL(stream_index).subblock_interleaver... % last entry (subblock_interleaver) is indexed with the CB index
...
K_pis = [UE_signaling.turbo_rate_matcher_UL(stream_index).subblock_interleaver.K_pi];
K_w = 3*K_pis;
UE_signaling.turbo_rate_matcher_UL(stream_index).total_bits_before_punturing = sum(K_w);


% Bit selection and pruning parameters
N_l    = UE_signaling.turbo_rate_matcher_UL(stream_index).N_l;
Q_m    = UE_signaling.MCS_and_scheduling_UL.CQI_params(stream_index).modulation_order;
G      = UE_signaling.turbo_rate_matcher_UL(stream_index).G;
G_p    = G /(N_l*Q_m);
gamma  = mod(G_p,C);
...
TB_segmentation = UE_signaling.turbo_encoder_UL(stream_index).encoded_CB_sizes; 
ratios = TB_segmentation/sum(TB_segmentation);

if LTE_params.balanced_rate_matching && C > 1
    G_p_C = round(G_p*ratios);
    difference = G_p-sum(G_p_C);
    G_p_C = G_p_C + difference/C;
    r = 0:(C-1);
    E_all = zeros(1,C);
    floored = (r<=(C-gamma-1));
    E_all(floored)  = N_l * Q_m * floor(G_p_C(floored));
    E_all(~floored) = N_l * Q_m * ceil(G_p_C(~floored));
    E_all = E_all+(G_p-sum(E_all)/N_l/Q_m)*Q_m;
else
...
for i=1:C
    UE_signaling.turbo_rate_matcher_UL(stream_index).total_bits_before_punturing = UE_signaling.turbo_rate_matcher_UL(stream_index).total_bits_before_punturing + 3*UE_signaling.turbo_rate_matcher_UL(stream_index).subblock_interleaver(i).K_pi;
    ...
    rv_idx = UE_signaling.turbo_rate_matcher_UL(stream_index).rv_idx;
    K_pi   = K_pis(i);
    R_tc   = UE_signaling.turbo_rate_matcher_UL(stream_index).subblock_interleaver(i).R_tc;
    F      = UE_signaling.TB_segmentation_UL(stream_index).F;
    r      = i-1;
    Nd     = UE_signaling.turbo_rate_matcher_UL(stream_index).subblock_interleaver(i).Nd;
    ...
    UE_signaling.turbo_rate_matcher_UL(stream_index).k_0_j_not_null{i} = k_0_j_not_null; % the actual rate-matching mapping, such as  ek{i} = wk{i}(k_0_j_not_null);
    UE_signaling.turbo_rate_matcher_UL(stream_index).ek_sizes(i)       = length(ek{i});
    UE_signaling.turbo_rate_matcher_UL(stream_index).null_pos{i}       = null_pos; % The <NULL> positions in wk{i}
    UE_signaling.turbo_rate_matcher_UL(stream_index).gamma             = gamma;
    UE_signaling.turbo_rate_matcher_UL(stream_index).G_p               = G_p;


    % Backwards compatibility for the receiver
    UE_signaling.turbo_rate_matcher_UL(stream_index).bit_selection_and_pruning_mapping{i} = k_0_j_not_null-1;
end
end
