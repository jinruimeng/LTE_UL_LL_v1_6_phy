...
% for i=1:length(ek)    %  Proc je tady ten cyklus? 
...
% %         BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).subblock_interleaver(i).K_pi,...
% %         BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).subblock_interleaver(i).R_tc,...
...
% %         i-1,...
% %         BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).subblock_interleaver(i).Nd,...
% %         2,...
% %         BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).null_positions{i});


    [bit_selection_and_pruning_mapping_TX k_0 null_positions] = LTE_common_turbo_rate_matcher_bit_selection_and_pruning_mapping(...
        BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).G,...
        N_IR,...
        BS_output.UE_signaling(1,uplink_user).MCS_and_scheduling(1,uplink_user).CQI_params(stream_index).modulation_order,...
        BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).N_l,...
        ...
        BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).null_positions);
         ...
         [bit_selection_and_pruning_mapping] = BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher_setting_from_tx; 
         sum(bit_selection_and_pruning_mapping_TX - bit_selection_and_pruning_mapping);
...
% Actual bit selection and pruning
    wk = LTE_common_turbo_rate_matching_bit_selection_and_pruning(double(ek),...
        ...
        BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).subblock_interleaver.K_pi*3);
            ...
            % kontrola LTE_common_turbo_rate_matching_bit_selection_and_pruning
            BS_output.UE_signaling(1,uplink_user).wk_rx = wk;
            sum(xor(BS_output.UE_signaling(1,uplink_user).wk_rx,BS_output.UE_signaling(1,uplink_user).wk_tx));
...
%     BS_output.UE_signaling(1,uplink_user).turbo_rate_matcher(stream_index).ek_sizes(i) = length(bit_selection_and_pruning_mapping);
