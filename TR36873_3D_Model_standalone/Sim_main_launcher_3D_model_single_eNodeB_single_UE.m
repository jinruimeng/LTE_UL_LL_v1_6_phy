%% 3GPP 3D channel model- main launcher file
...
% network geometry
config.min_UE_eNodeB_distance                          = 35;     %[m]  %--> 3D_UMa: 35 [m], 3D_UMi: 10 [m]
config.inter_eNodeB_distance                           = 500;
config.nr_eNodeB_rings                                 = 1;
config.nr_sectors                                      = 1;
config.antenna_azimuth_offsett = 60;
...
config.UE_antenna_slant_angle                        = 0;     % if ULA --> slant angle is 0�; if 'XPOL' --> slant_angle is 90�
...
config.antenna.slant_angle                           = 0; % If 'COPOL' --> slant_angle is 0�, 'XPOL' --> slant_angle is 45�
...
config.simulation_time_blocks       = 20;

% load further parameters that don't need to be changed
config = load_specific_params(config);

%% generate network layout
fprintf('Generating eNodeB positions');
% eNodeB_site_positions = network_geometry.single_eNodeB(config);
...
while networkClock.current_block < config.simulation_time_blocks %ADDED
     ...
     fprintf('Generating desired and interfering channels for UE %d \n', eNodeBs(bb).attached_UEs_vector(uu).id);
            if eNodeBs(bb).attached_UEs_vector(uu).recalculate_3D_smallscale_fading
                ...
                if eNodeBs(bb).attached_UEs_vector(uu).is_indoor
                    ...
                    if eNodeBs(bb).attached_UEs_vector(uu).is_LOS
                ...
                pathloss_linear_0 = networkPathlossMap.pathloss(eNodeBs(bb).attached_UEs_vector(uu).pos_pixel(1,1),eNodeBs(bb).attached_UEs_vector(uu).pos_pixel(1,2),bb); 
...
%             eNodeBs(bb).attached_UEs_vector(uu).channels = [];
            % INTERFERING CHANNELS
            
            if isempty(eNodeBs(1).neighbors_eNodeB)
                fprintf('There are no interferes in this scenario')
            else
...
save(fullfile(config.results_folder,results_filename),'config','networkPathlossMap','UEs','eNodeBs','eNodeB_sites');
