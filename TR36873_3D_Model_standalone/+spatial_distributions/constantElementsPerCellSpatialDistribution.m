    ...
    function elements_positions = generate_positions(obj,varargin)
            ...
            for s_idx = 1:number_of_sectors
                if elements_sector(s_idx)~=0
                    [row,col] = find(networkPathlossMap.sector_assignment(:,:)==s_idx & networkPathlossMap.excluding_region_near_BS(:,:,s_idx)==1);
...
%                 for j_=1:size(positions_pixels_per_cell{i_},1)
%                     position_pixels(l_,:) = positions_pixels_per_cell{i_}(j_,:);
%                     if position_pixels(l_,1) > size(obj.networkPathlossMap.pathloss,1)
%                         position_pixels(l_,1) =  position_pixels(l_,1) - (position_pixels(l_,1)- size(obj.networkPathlossMap.pathloss,1)) -1;
%                     end
%                     if position_pixels(l_,2) > size(obj.networkPathlossMap.pathloss,2)
%                         position_pixels(l_,2) =  position_pixels(l_,2) - (position_pixels(l_,2)- size(obj.networkPathlossMap.pathloss,2)) -1;
%                     end
%                     l_=l_+1;
%                 end
%             end
