function [...
    ...
    cell_sizes,...
    cell_centers...
    ] = LTE_common_calculate_cell_capacity(config,networkPathlossMap,sites,eNodeBs,varargin)
...
if shadow_fading_used && config.debug_level>=1
...
% if strcmp(config.femtocells_config.mode, 'CSG')
...
thermal_noise_W = 10^(config.UE.thermal_noise_density/10) / 1000 * config.bandwidth * 10^(config.UE.receiver_noise_figure/10);
...
if config.add_femtocells
    switch config.femtocells_config.mode
        case 'OSG'
        ...
        case 'CSG'
            ...
            if tot_eNodeBs > N_macro_eNodeBs % there are femto sectors
                ...
                indoor_areas_ = (networkPathlossMap.distances < config.femtocells_config.macroscopic_pathloss_model_settings.indoorAreaRadius);
...
bandwidth       = config.N_RB*config.RB_bandwidth;
CP_length_s     = config.CP_length_samples/config.fs;
symbol_length_s = config.BF_length/(config.N_sym*2);
...
subframe_size_Sym = config.N_sym*subcarriers_per_RB*2*config.N_RB;       % 2 for 2 slots (2x0.5 ms)

RefSym_ratio  = 1-(nRef_sym / (config.N_sym*subcarriers_per_RB*nTXantennas)); % Ratio of reference_symbols/total_subframe_symbols
