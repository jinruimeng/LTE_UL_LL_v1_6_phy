function [ eNodeB_positions ] = hexagonal_eNodeB_grid(LTE_config)
...
if isfield(LTE_config,'exclude_dominant_interferer')
   if LTE_config.exclude_dominant_interferer
      fprintf('Hex-grid generation: Dominant interferer is not taken into account... \n');
      [~, max_index] = max(tmp_x);
      tmp_x(max_index) = [];
      tmp_y(max_index) = [];
   end
