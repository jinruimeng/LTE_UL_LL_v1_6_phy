...
function pred_mse_user( simulation_results, LTE_params )
...
for u = 1:N_UE
    if simulation_results.conf_interval_probability == 0 || isempty(fieldnames(UE_spec(u).confidence))
        ce_tmp = UE_spec(u).channel_pred_error;
        ce(u, :) = mean(ce_tmp,1); 

    else
        ce(u, :) = UE_spec(u).confidence.channel_pred_error(1,:);
        ce_confidence.lower(u, :) = UE_spec(u).confidence.channel_pred_error(2,:);
        ce_confidence.upper(u, :) = UE_spec(u).confidence.channel_pred_error(3,:);
...
for u = 1:N_UE
    ...
    semilogy( simulation_results.SNR_vector(1, :), ce(u, :), ...
    ...
    if simulation_results.conf_interval_probability > 0 % confidence intervals computed
        for s = 1:N_snr
            plot( simulation_results.SNR_vector(1,s) * ones(1,2), ...
