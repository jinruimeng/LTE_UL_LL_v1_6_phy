classdef video_old < traffic_models.generic_tm
...
%             generate_packet(sum(obj.slices),current_TTI);
%             obj.data_buffer = obj.data_buffer+sum(obj.slices); % if there are still slices left put them in the buffer now, because a new frame is generated.
            obj.generate_slices;
            obj.generate_inters;
            generate_packet(sum(obj.slices),current_TTI);
            obj.slices(1) = 0; % data is now in the buffer
            ...
            if ~isempty(ind)
                obj.data_buffer = obj.data_buffer+obj.slices(ind);
                obj.slices(ind) = 0;
            end
        end        
    end
