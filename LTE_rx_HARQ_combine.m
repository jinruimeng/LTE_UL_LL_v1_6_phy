function [ d_HARQ BS ] = LTE_rx_HARQ_combine(d,BS,BS_ue_specific,stream_index)
...
if rv_idx==0
    BS.HARQ_rx_soft_buffer{HARQ_process_idx,stream_index} = d;
    d_HARQ = d;
else
    % If not, then add to the buffer each codewords (max. 6144 bits)
    for ii=1:length(d)
        BS.HARQ_rx_soft_buffer{HARQ_process_idx,stream_index}{ii} = BS.HARQ_rx_soft_buffer{HARQ_process_idx,stream_index}{ii} + d{ii};
    end
    d_HARQ = BS.HARQ_rx_soft_buffer{HARQ_process_idx,stream_index};
end
