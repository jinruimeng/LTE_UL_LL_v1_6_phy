...
%% General
LTE_params.simulation_method    = 'normal';   % 'parallel' or 'normal' to parallelize the SNR loop in LTE_sim_main_parallel.m
LTE_params.nUE                  = 1;            % number of user equipments to simulate
LTE_params.nBS                  =1;            % number of base stations to simulate
LTE_params.Bandwidth            = 20e6;        % in Hz, allowed values: 1.4 MHz, 3 MHz, 5 MHz, 10 MHz, 15 MHz, 20MHz => number of resource blocks 6, 15, 25, 50, 75, 100
...
LTE_params.DFT_spreading_off            = true;    % turn off DFT spreading to have OFDM performance

%% Postprocessing
LTE_params.confidence_interval_probability = 0;  % probability that the result is in the computed 
                                                    % confidence interval. to disable confidence
                                                    % intervals set it to 0. (default value 0.95 [allowed values: 0 < x < 1])

%% Plots
LTE_params.show_plots   = true;
...
LTE_params.UE_config.user_speed             = user_speed;                    % the UE speed defines the doppler spread and the time selectivity of the channel[m/s]
...
LTE_params.BS_config.nRX                                = 1;            % number of BS receive antennas {1,2,4}
LTE_params.BS_config.receiver                           = 'MMSE';     % 'ZF', 'MMSE', 'IAMMSE'

LTE_params.BS_config.channel_estimation_method          = 'LS_AV';    % 'PERFECT','LS_AV','LS_SAV', 'LS_QS', 'DFT', 'MMSE','MMSE_2D'
...
LTE_params.BS_config.channel_est_frequency_smoothing    = [0,8,15,15,30,30,60,60];   % frequency smoothing constraint for LS_QS channel estimation
...
LTE_params.ChanMod_config.type                  = type;                   % 'PedA', 'PedB', 'PedBcorr', 'AWGN', 'flat Rayleigh', 'VehA', 'VehB', 'TU', 'RA', 'HT', 'winner_II', 'ePDP', 'TR 36.873'
LTE_params.ChanMod_config.filtering             = 'BlockFading';                % 'BlockFading', 'FastFading'
LTE_params.ChanMod_config.time_correlation      = 'correlated';                % 'independent', 'correlated'
...
% General settings
LTE_params.TR_36_873.environment = 'UMi'; % 'UMa' or 'UMi'
...
LTE_params.TR_36_873.UE_pos_rand = false; % true or false
LTE_params.TR_36_873.eNodeB_pos = [0, 0]; % in meters, size [LTE_params.nBS, 2]
% LTE_params.TR_36_873.eNodeB_pos = [0, 0; 500, 0]; % in meters, size [LTE_params.nBS, 2]
LTE_params.TR_36_873.UE_pos = [-10, 0]; % in meters, size [LTE_params.nBS*LTE_params.nUE, 2]
...
LTE_params.TR_36_873.eNodeB_heights = repmat(1.5, [1, LTE_params.nBS]); % in meters, size [1, LTE_params.nBS]
