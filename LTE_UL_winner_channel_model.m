function [channel_out, delays, out] = LTE_UL_winner_channel_model(N_subframes,Arrays,varargin)
...
% www.nt.tuwien.ac.at
%
% [1]   IST-WINNER D1.1.2 P. Ky�sti, et al., "WINNER II Channel Models", ver 1.1, Sept. 2007. 
...
wimpar.SampleDensity            = LTE_params.speed_of_light/(2*LTE_params.carrier_freq_UP*Channel_Sampling_Time*LTE_params.UE_config.user_speed);           %BUGFIX carrier_freq_UP
...
wimpar.CenterFrequency          = LTE_params.carrier_freq_UP;                                                  % carrier frequency in Herz [ {5.25e9} ]
...
MsAAIdx = ones(1,LTE_params.nUE*LTE_params.nBS);   %every user is using antenna defined Arrays(1)
BsAAIdxCell = {[2]};    %   base station is using antenna defined Arrays(2)

layoutpar=layoutparset(MsAAIdx,BsAAIdxCell,LTE_params.nUE*LTE_params.nBS,Arrays);

layoutpar.ScenarioVector = LTE_params.ChanMod_config.winner_settings.Scenario*ones(1,LTE_params.nUE*LTE_params.nBS);    %   1=A1, 2=A2, 3=B1, 4=B2, 5=B3, 6=B4, 7=B5a, 8=B5c, 9=B5f, 10=C1,
...
switch LTE_params.ChanMod_config.winner_settings.PropagCondition
    case 'LOS'
        layoutpar.PropagConditionVector =1*ones(1,LTE_params.nUE*LTE_params.nBS);  %   (NLOS=0/LOS=1)
    case 'NLOS'
        layoutpar.PropagConditionVector =0*ones(1,LTE_params.nUE*LTE_params.nBS);  %   (NLOS=0/LOS=1)
end
...
for user_i = 1:LTE_params.nUE*LTE_params.nBS
    ...
    for tap_i = 1:channel_matrix_size(3)
        tap_positions = find(delays(1,:) == tap_i-1);
    ...
    channel_out{user_i} = sqrt(LTE_params.BS_config.nRX * LTE_params.UE_config.nTX) * channel_out{user_i} / sqrt(channel_energy);       % channel is normalized to have mean energy Nt*Nr
end
